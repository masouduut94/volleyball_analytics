...
from schemas import TeamBaseSchema, NationBaseSchema, CameraBaseSchema, SeriesBaseSchema, RallyBaseSchema, MatchBaseSchema, VideoBaseSchema
...
if __name__ == '__main__':
    ...
    c1 = CameraBaseSchema(angle_name='Rear_1')
    camera = Camera.save(c1.model_dump())

    se1 = SeriesBaseSchema(host='gdansk', start_date=datetime.now(), end_date=datetime.now())
    ...
    v1 = VideoBaseSchema(path=video_path.as_posix(), camera_type=camera.id)
    video = Video.save(v1.model_dump())

    m1 = MatchBaseSchema(team1_id=team1.id, team2_id=team2.id, series_id=se.id, video_id=video.id)
    ...
    rally = RallyBaseSchema(id=None, match_id=1, sets={}, spikes=spikes, blocks={}, ball_positions={}, team1_positions={},
